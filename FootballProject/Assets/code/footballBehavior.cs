...
public class footballBehavior : MonoBehaviour
{
    ...
    void tagSwitch()
    {
        ...
        {
            if(defenseTarget.parent != null)
            {
                if(defenseTarget.parent.parent != null)
                {
	...
	void Update ()
    {
        ...
        if(transform.tag == "BallCarrier")
        {
            ...
            float rampedSpeed = speed * (targetOffset.magnitude / dist);
            float clippedSpeed = Mathf.Min(rampedSpeed, speed);
            Vector3 desiredVelocityA = (clippedSpeed / targetOffset.magnitude) * targetOffset;
            desiredVelocityA.y = 0;
            //rb.AddForce(desiredVelocity - rb.velocity);
            ...
            Vector3 desiredVelocityE = Vector3.zero;
            if (evadeTarget != null)
            {
                ...
                if (dist > acceptableDistance)
                {
                    ...
                    desiredVelocityE = -(speed * (futurePosition - transform.position).normalized);
                    //rb.AddForce(desiredVelocity - rb.velocity);
                }
                else
                {
                    speed = burstSpeed;
                    desiredVelocityE = -(speed * (evadeTarget.position - transform.position).normalized);
                   // rb.AddForce(desiredVelocity - rb.velocity);
                }
            }

            rb.AddForce((desiredVelocityA - rb.velocity) + (desiredVelocityE - rb.velocity));
